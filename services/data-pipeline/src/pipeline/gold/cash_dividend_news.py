"""
Reads filed cash dividends shaped as feed items out of the published Gold snapshot.
"""
...
def window_for(trade_date: date) -> tuple[date, date]:
    """The feed-date range a run projects, matching the Gold model's own window."""
...
def read_items(trade_date: date, settings: Settings | None = None) -> list[dict[str, Any]]:
    """Cash-dividend feed items in the run's window, empty when Gold has none yet."""
...
def main() -> None:
    ...
    print(f"{len(items)} cash-dividend feed items in window for {today}")
    for item in items:
